...
async function runAudit() {
  ...
  // Process only one lead per run (can be looped later)
  ...
  // Estimate PITIA
  // Standard scenario: 20% down, 30-year fixed, 7.1% interest
    ...
    html: `
      <div style="font-family: sans-serif; max-width: 600px; margin: auto; border: 1px solid #ddd; padding: 20px; border-radius: 10px;">
        ...
        <p style="color: #6c757d; font-size: 12px; text-align: center;">
          ...
          Preview the landing page: <a href="https://www.capitalbridgesolutions.com/audits/${lead.id}">View Audit Page</a>
        </p>
      </div>
    `,
  };
  ...
  try {
    ...
    console.log(`✅ Status updated to 'audited' in leads.json for ${lead.id}`);
